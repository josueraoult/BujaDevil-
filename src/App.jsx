// App.jsx – BujaDevil Blog (pitch.com/cocacola design, bugfixé, transition fluide, UX 2025)
// Stack: React + Firebase + Tailwind, tout-en-un, SPA, admin, dark mode, animations modernes
...
function Header({ onNavigate, page, isAdmin, onLogout }) {
  return (
    <header className="sticky top-0 z-40 bg-[#101010]/90 blur-bg px-2 md:px-0 flex items-center justify-between h-20 border-b border-[#18181b] shadow-sm transition-all duration-300">
      <div className="flex items-center group">
        <img src="/logo512.png" alt="logo" className="w-10 h-10 rounded-xl mr-3 drop-shadow-xl group-hover:scale-105 transition-transform duration-200" />
      ...
      <nav className="flex gap-2 md:gap-6 items-center">
        <button
          ...
          className={`font-semibold text-base py-1 px-3 rounded-full transition-colors ${page === "home" ? "bg-[#ff003c] text-white shadow-lg" : "hover:bg-[#232323] text-[#bbb]"}`}
        >
          Accueil
        </button>
        {isAdmin ?
          <button onClick={() => onNavigate("admin")}
            className={`font-semibold text-base py-1 px-3 rounded-full transition-colors ${page === "admin" ? "bg-[#07f8ff] text-black shadow-lg" : "hover:bg-[#232323] text-[#bbb]"}`}>
            Admin
          </button>
        : null}
        <a
          ...
          className="text-[#bbb] hover:text-[#ff003c] font-semibold text-sm transition-all"
        >
          GitHub
        </a>
        {isAdmin && onLogout &&
          <button onClick={onLogout} className="ml-3 btn shadow-lg hover:scale-105 transition">Déconnexion</button>
        }
      </nav>
    </header>
  );
...
function HomePage({ posts, onOpenPost, filter, setFilter }) {
  return (
    <main className="max-w-5xl mx-auto py-8 px-2 fade-in">
      <h1 className="text-4xl md:text-5xl font-black mb-2 text-shadow tracking-tight">La Crème Du Gaming</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`btn ${filter === cat ? "bg-[#07f8ff] text-black" : "bg-[#18181b] text-[#bbb] hover:bg-[#101010]"}`}>
            {cat}
          </button>
        ))}
      </div>
      <section className="grid-autofill">
        {posts.length === 0 ? (
          ...
          <PostCard key={post.id || i} post={post} onClick={() => onOpenPost(post)} />
        ))}
      </section>
    </main>
  );
...
function PostCard({ post, onClick }) {
  return (
    <div className="card cursor-pointer hover:scale-[1.03] transition-all fade-in group" onClick={onClick} tabIndex={0} aria-label={post.title} title={post.title}>
      {post.thumbnailUrl && (
        <img src={post.thumbnailUrl} alt="" className="w-full h-48 object-cover rounded-xl mb-3 group-hover:shadow-2xl transition-shadow" loading="lazy" />
      )}
      ...
      <h2 className="text-xl font-extrabold mt-1 mb-0.5 group-hover:text-[#ff003c] transition">{post.title}</h2>
  ...
  return (
    <section className="max-w-5xl mx-auto py-8 px-2 fade-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="btn shadow-lg hover:scale-105" onClick={onAdd}>+ Nouvel Article</button>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-[#f6f6f6] bg-[#18181b] rounded-2xl shadow">
          ...
          <tbody>
            {posts.map(post => (
              <tr key={post.id} className="border-b border-[#232323] hover:bg-[#232323] transition">
...
export default function App() {
  ...
  // -- RENDU PRINCIPAL
  return (
    <div className="min-h-screen bg-[#101010] font-inter">
      <Header
